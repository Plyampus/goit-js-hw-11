...
</li>`}function f(a){return a.map(g).join("")}const n=document.querySelector(".form"),c=document.querySelector(".gallery"),p=document.querySelector(".loader");n.addEventListener("submit",a=>{a.preventDefault(),c.innerHTML="";const s=a.target.elements.value.value;s?(p.classList.remove("is-hidden"),u(s).then(r=>{r.hits.length||o.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!"});const i=f(r.hits);c.insertAdjacentHTML("afterbegin",i),p.classList.add("is-hidden")}).then(()=>{new m(".gallery a",{captionsData:"alt",captionDelay:250})}).catch(r=>console.log(r))):o.error({title:"Error",message:"The search field is empty. Please try again!"}),n.reset()});
