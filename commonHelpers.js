...
</li>`}function g(s){return s.map(f).join("")}const o=document.querySelector(".form"),c=document.querySelector(".gallery"),u=document.querySelector(".loader");o.addEventListener("submit",s=>{s.preventDefault(),c.innerHTML="";const a=s.target.elements.value.value;a.trim()?(u.classList.remove("is-hidden"),d(a).then(r=>{r.hits.length||n.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!"});const i=g(r.hits);c.insertAdjacentHTML("afterbegin",i),u.classList.add("is-hidden")}).then(()=>{new m(".gallery a",{captionsData:"alt",captionDelay:250})}).catch(r=>console.log(r))):n.error({title:"Error",message:"The search field is empty. Please try again!"});try{if(!s.target.elements.value.value.trim())throw new Error("Поле вводу не може бути порожнім.")}catch(r){alert("Сталася помилка: "+r.message)}window.addEventListener("offline",function(){alert("Втрата з'єднання з Інтернетом.")}),o.reset()});
